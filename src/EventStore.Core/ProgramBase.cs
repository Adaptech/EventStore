...
// ReSharper restore StaticFieldInGenericType
        ...
        private void CommitSuicideIfInBoehmOrOnBadVersionsOfMono(TOptions options)
        {
            if(!options.Force)
            {
                ...
                if(OS.IsUnix && !(OS.GetRuntimeVersion().StartsWith("4.6.2") || OS.GetRuntimeVersion().StartsWith("3.12.1")))
                {
                    Log.Warn("You appear to be running a version of Mono which is untested and unsupported. Only Mono 4.6.2 or Mono 3.12.1 is supported at this time.");
                }
            }
        }
